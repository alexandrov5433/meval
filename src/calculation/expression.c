 ...
 * @param exp Pointer to a String (galxlib/gstring) containing the expression, which is the main expression for this instance.
 * The memory of this pointer must not be freed, as it is  placed in the new Expression instance.
 * @param parent_expression Pointer to an other Expression, in which this new one will be contained.
 * The memory of this pointer must also not be freed. NULL can be given as an argument, if there is no parent Expression.
 * @param output_ext A double Expression pointer where the new Expression pointer will be placed. 
 * @return 0 on succees. On failure:
 * 
 * 1 if the exp argument is NULL.
 * 
 * 2 if memory allocation (malloc) for a new Expression failed.
 */
int *new_expression(String *exp, Expression *parent_expression, Expression **output_exp)
{
	if (exp == NULL) // parent_expression can be NULL
		return 1;

	Expression *new_exp = malloc(sizeof(Expression));
	if (new_exp == NULL)
		return 2;
	
	new_exp->exp = exp;
	new_exp->inner_expressions = NULL;
	new_exp->parent_expression = parent_expression; // can be NULL
	new_exp->value = 0.0;
 ...
 */
int delete_inner_exp_by_ptr(Expression *const exp_container, Expression *exp_to_delete)
{
	if (exp_container == NULL || exp_to_delete == NULL)
		return 1;

	if (exp_container->inner_expressions == NULL)
		return 2;
 ...
 */
static void _free_expression(Expression *ptr)
{
	if (ptr == NULL)
		return;

	if (ptr->inner_expressions != NULL)
	{
		for (int i = 0; i < ptr->inner_expressions->length; i++)
			_free_expression((ptr->inner_expressions->array)[i]);

		free(ptr->inner_expressions);
	}
...
void free_expression(Expression *ptr)
{
	...
	if (ptr->parent_expression != NULL)
		ptr->parent_expression->inner_expressions = deleteExpFromArray(ptr, ptr->parent_expression->inner_expressions);

	_free_expression(ptr);
}

static void add_inner_expression(Expression *main_exp, Expression *inner_exp)
{
	if (main_exp->inner_expressions == NULL)
	{
		main_exp->inner_expressions = newExpressionArray(inner_exp);
		return;
	}
	Expression **tmp_ref = main_exp->inner_expressions->array;

	Expression **newArray = realloc(tmp_ref, main_exp->inner_expressions->length + 1);
	if (newArray == NULL)
	{
	...
	main_exp->inner_expressions->array = newArray;
	(main_exp->inner_expressions->array)[main_exp->inner_expressions->length] = inner_exp; // old length is now the last index
	main_exp->inner_expressions->length++;						  // increment old length, to actually show length
}
